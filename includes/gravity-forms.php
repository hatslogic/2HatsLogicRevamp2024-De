...
add_filter( 'gform_field_css_class', 'app_custom_gforms_classes', 10, 3 );
function app_custom_gforms_classes( $classes, $field, $form ) {
...
function app_get_forms() {
	...
	// app_gravity_form_options filter - allows you to modify the form list
...
function app_render_gform( $id, $is_ajax = false, $args = array() ) {
	...
	$step = apply_filters( 'theme_gform_tabindex_step', 500 );
...
function app_gform_ajax_spinner_url( $image_src, $form ) {
	return get_bloginfo( 'stylesheet_directory' ) . '/resources/images/spinner.svg';
}

// Replace input submit button with button tag
add_filter( 'gform_submit_button', 'app_form_submit_button', 10, 2 );
function app_form_submit_button( $button, $form ) {
	ob_start();
	?>
		<button class='button gform_button form__submit' id='gform_submit_button_<?php echo $form['id'] ?>'>
		    <div class='form__submit-inner'>
		    	<span><?php echo $form['button']['text'] ?></span>
